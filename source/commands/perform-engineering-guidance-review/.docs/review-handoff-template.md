...
The reviewer holds closure authority — the worker never decides when reviewing stops. A held Blocker or Concern stays in the loop while another exchange could clarify or resolve it. When the reviewer and implementer reach a fundamental impasse, the reviewer raises it in the `Human Escalation` section and the loop pauses until you decide.
...
2. **Adjudicate the pushbacks.** Where the implementer declined or deferred a finding, weigh the reasoning. Withdraw the finding if the reasoning holds, or hold it if it doesn't. Never silently drop a Blocker or Concern.
