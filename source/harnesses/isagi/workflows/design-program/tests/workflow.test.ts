...
test('command captures the story, predecessor paths, program-design path, and repository path', async () => {
  ...
  assert.deepEqual(
    ...
    ['story', 'currentStatePath', 'architecturePath', 'artifactPath'],
  );
...
test('spawns the configured Fable writer with the program design skill and required footer', async () => {
...
test('judges every writer turn and fails when the initial program design is incomplete', async () => {
  ...
  assert.match(harness.feedback.at(-1)?.message ?? '', /did not produce a reviewable program design/);
...
test('a ready writer starts the independently configured program design reviewer', async () => {
  const harness = workflowHarness();
  const current = baseState({
    kind: 'await_initial_writer_judgment',
    writer: agent(11, 21),
    writerResponse: 'Program design complete.',
    mode: 'normal',
  });
  ...
  assert.match(prompt, /Architecture: scratch\/architecture/);
  assert.match(prompt, /\*\*Contradictions:\*\*/);
  assert.match(prompt, /\*\*Important Simplifications:\*\*/);
  assert.match(prompt, /\*\*Missing Program Decisions:\*\*/);
  ...
  assert.match(prompt, /Do not demand exhaustive file-change inventories/);
  assert.match(prompt, /attempting to reconstruct changes between review rounds/);
...
test('retry judges a newer complete writer turn without sending another prompt', async () => {
  const harness = workflowHarness({
    ...
    histories: { 11: [message('assistant', 'Program design is complete and verified.')] },
  });
  const result = await workflow.step(
    harness.ctx,
    baseState({
  ...
  assert.equal(result.type, 'suspend');
  assert.equal(harness.sent.length, 0);
  assert.match(harness.headless[0]?.prompt ?? '', /Program design is complete and verified/);
...
test('a failed retry recheck sends one continuation and a second failure remains terminal', async () => {
  const histories: Record<number, readonly WorkflowConversationMessage[]> = {
    11: [message('assistant', 'Program design is now complete.')],
  };
...
test('review findings are judged and sent to the persistent writer', async () => {
  const review = 'Timeout behavior is unresolved.\n\n## Human Escalation\n\nNo escalation.';
  ...
  assert.match(harness.sent[0]?.prompt ?? '', /Timeout behavior is unresolved/);
  assert.match(harness.sent[0]?.prompt ?? '', /Correct a predecessor artifact only/);
...
test('a completed revision is judged before its response goes to the reviewer', async () => {
  const writerResponse = 'Clarified timeout behavior and retained one evidence-backed pushback.';
  const harness = workflowHarness({
  ...
  assert.match(prompt, /\*\*Other Significant Issues:\*\*/);
  assert.match(prompt, /Do not demand exhaustive file-change inventories/);
  assert.match(prompt, /Reread the current artifacts/);
  assert.match(prompt, /repeatedly disagreed/);
  assert.equal(prompt.endsWith(PROMPT_FOOTER), true);
...
test('review completion closes both workflow-created panes and returns the program design path', async () => {
  const harness = workflowHarness();
  const result = await workflow.step(
    harness.ctx,
    baseState({
