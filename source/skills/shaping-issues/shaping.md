...
Shape epics and stories around value that becomes available through the chosen increment rather than the engineering route or completeness of an eventual architecture. Preserve enough reasoning to help later collaborators challenge earlier technical ideas without losing the goal those ideas served.

## Epic Contract

An epic has a clear product goal, why that goal matters, a coherent scope, and a completion condition. Its supporting context adapts to the conversation and may include:
...
Keep the epic mostly stable after shaping. Revise it when its product direction, boundary, completion condition, or story set changes materially. Keep routine progress and engineering execution history in the story's working context or temporary plan.
...
## Candidate Epic Contract

A candidate epic preserves a possible later product outcome, why it may matter, why it was deferred, and whatever relationships, activation signals, or originating reasoning would be expensive to recover. Adapt its supporting context to the conversation.

A candidate epic has no stories and makes no implementation commitment. It may later change, merge, or disappear. When it becomes relevant, use it as input to a fresh shaping discussion rather than promoting it directly to a hardened epic.

## Sequencing the Direction

In Sharpen, identify the coherent promise that provides worthwhile value now and separate later increments into candidate epics when preserving them is useful. A valuable increment may be a product experience, operational improvement, learning result, or enabling capability that unlocks immediate use.

Let the human choose how much sequence to preserve: the active epic alone, its likely successor, or a broader candidate roadmap. Make dependencies and accepted rework visible when they affect that choice. Treat deferred candidates as options whose value and order can change, not as obligations.

## Story Contract

A story names one coherent product, operational, learning, or enabling outcome under its parent epic. It provides observable value or unlocks immediate use, has acceptance criteria, and carries the branch-local context that later brainstorming cannot cheaply recover from the epic or repository.

Useful story context may include:

- Why the story exists and how it advances the epic.
...
Treat the story as a starting hypothesis. Later brainstorming may choose a different engineering route while preserving the story when its outcome and acceptance criteria still hold. Use this skill again when the product outcome, acceptance criteria, or epic story set needs reshaping.
...
State the uncertainty, the decisions it should enable, and the epic or downstream stories that will consume the result. Its acceptance criteria cover a usable verdict and the resulting reconciliation:
...
- Using this skill, reconcile the conclusions into the parent epic and every affected downstream story.

Reconciliation may confirm, revise, add, remove, split, or merge stories, or reshape the epic itself.
...
When coherent stories remain hard to shape, return to the epic's product goal, boundaries, completion condition, or sequence and sharpen the missing context.

## Quality Checks

Before presenting or persisting shaped artifacts, reread the epic and its stories as a future collaborator without the conversation. Check that every story is necessary to the now promise and traceable to its originating reasoning, and that each known consequential uncertainty is visible in the epic or assigned to an exploration story. Check that candidate epics remain uncommitted seeds without stories.
