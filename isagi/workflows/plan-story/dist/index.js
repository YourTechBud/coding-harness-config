...
function parseSlug(output) {
  const record = parseExactObject(output, ["slug"], "slug judgment");
  if (typeof record.slug !== "string") {
    throw new Error("slug judgment field slug must be a string.");
  }
  const slug = record.slug.trim();
  if (slug.length > 63 || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) {
    throw new Error(
      "slug judgment field slug must be under 64 characters and contain lowercase ASCII letters, digits, and single hyphens."
    );
  }
  return slug;
}
    ...
    switch (state.stage.kind) {
      ...
      case "await_plan_directory": {
        try {
          ...
          return i(withStage(state, { kind: "start_current_state", plan }));
        } catch (error) {
          return failWorkflow(
            ctx,
      ...
      case "await_review_approval": {
        ...
        return i(withStage(state, { kind: "spawn_planner", plan: state.stage.plan }));
      }
      case "spawn_planner": {
...
function choosePlanContext(repositoryPath, slug) {
  let candidate = slug;
  let suffix = 2;
...
function childArtifactError(incoming, input) {
  ...
  if (!Number.isInteger(record.reviewCount) || record.reviewCount < 1) {
    return `${input.workflowKey} child workflow ${input.runId} returned an invalid review count.`;
  }
  return null;
}
function childStoryReviewError(incoming, runId, expected) {
...
function planArtifactError(repositoryPath, plan) {
  ...
  const phaseFiles = readdirSync(directoryPath).filter(
    (name) => /^phase-\d{2}-.+\.md$/.test(name)
  );
